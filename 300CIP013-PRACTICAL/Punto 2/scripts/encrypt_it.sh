#!/usr/bin/env bash
# scripts/encrypt_it.sh
# Usage: ./encrypt_it.sh <rotation> <phrase...>
set -euo pipefail
...
# normalize rotation into [0,25]
mod() { local a=$1 b=$2; local r=$((a % b)); [[ $r -lt 0 ]] && r=$((r + b)); echo $r; }
k=$(mod "$k" 26)

# Use awk to rotate letters; keep non-[A-Za-z] unchanged (accents preserved as-is)
awk -v K="$k" '
